...
d=$(docker buildx imagetools inspect ghcr.io/busnes-app/ky_server_base:$sha --format '{{.Manifest.Digest}}') \
  && gh attestation verify "oci://ghcr.io/busnes-app/ky_server_base@$d" --repo Busnes-app/ky_server_base \
       --cert-identity https://github.com/Busnes-app/ky_server_base/.github/workflows/ci.yml@refs/heads/master \
  && [ "$(gh attestation verify "oci://ghcr.io/busnes-app/ky_server_base@$d" --repo Busnes-app/ky_server_base \
       --cert-identity https://github.com/Busnes-app/ky_server_base/.github/workflows/ci.yml@refs/heads/master \
  ...
  && (umask 077; t=$(mktemp ./.env.XXXXXX) && touch .env && { grep -v '^KY_IMAGE=' .env || [ $? -eq 1 ]; } > "$t" \
      && echo "KY_IMAGE=ghcr.io/busnes-app/ky_server_base@$d" >> "$t" && mv "$t" .env) \
  && grep -qxF "KY_IMAGE=ghcr.io/busnes-app/ky_server_base@$d" .env
```
...
printf '%s\n' "$imgs" | grep -qxF "ghcr.io/busnes-app/ky_server_base@$d" || printf '%s\n' "$imgs" | grep -qxF 'ky_server_base:local' \
